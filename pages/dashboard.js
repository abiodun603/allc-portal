import { Avatar,Box,Grid, Modal, Tooltip } from '@mui/material'
...
import { getAuthEmail, getAuthToken } from '../redux/features/authSlice'
import { fetchAsyncTask, getAllTask } from '../redux/features/taskSlice'
...
const LeftComponent = () => {
    ...
    return(
        <div>
            ...
            <div className="grid gap-x-6 gap-y-4 grid-cols-3 mt-16">
                {
                    task.map((item, index) => {
                        return (
                            <div key={index}>
        ...
        <div className='mt-8'>
          {
            tasks.map((task) => {
              return (
                <div className="flex flex-col mb-3  " key={task.id}>
                  <div className="bg-klightgreen w-full py-3 px-3 rounded-lg ease-out duration-500 hover:scale-105">
                    ...
                    <div className='flex items-center justify-center w-fit rounded-full mt-4 h-[30px] px-6 bg-white ease-out duration-500 hover:scale-110 cursor-pointer'>
                      <p className='font-normal text-sm'>{task.status}</p> 
                    </div>
                  </div>
                </div>
              )
...
const Dashboard = () => {
  ...
  useEffect(() => {
    dispatch(fetchAsyncTask({token, email}))
  }, [])
